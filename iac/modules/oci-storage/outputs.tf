...
output "state_bucket_name" {
  ...
  value       = data.oci_objectstorage_bucket.tfstate.name
}

output "state_bucket_url" {
  description = "Full URL to the Terraform state bucket"
  value       = "https://${var.namespace}.compat.objectstorage.ca-toronto-1.oraclecloud.com/${data.oci_objectstorage_bucket.tfstate.name}"
}
